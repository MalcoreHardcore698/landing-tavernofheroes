...
$(function(){function o(){$(document).width()>=992?$(".lower-col").css({height:$(".important-news").height()-$(".upper-col").height()}):$(document).width()<992&&$(".lower-col").css({height:"100%"})}$(".primary-content .slider .owl-carousel").owlCarousel({items:1,dot:!0,autoplay:!0,mouseDrag:!1,loop:!0}),$("#mobile-menu").mmenu({navbar:{title:'<a href="/" class="logo-st"><img src="img/logo/ToHLogo3.1.png" alt="Logo Tavern of Heroes"></a>'},offCanvas:{position:"right"}},{classNames:{fixedElements:{fixed:"mobile-menu"}}}),o(),$(window).resize(function(){o()}),$("header .mobile-tailings-menu .top-arrow").css({height:$(window).height()});$(window).scroll(function(){$(window).scrollTop();$(window).scrollTop()>130?$("header .hamburger-d").css({position:"fixed",top:"15px",right:"15px",boxShadow:"0px 0px 10px 0px rgba(0,0,0,.1)"}):$("header .hamburger-d").css({position:"relative",top:"0",right:"0",boxShadow:"0px 0px 3px 0px rgba(0,0,0,.05)"})}),$("header .mobile-tailings-menu .top-arrow").click(function(){$("body").animate({scrollTop:0},500,"swing")})});